...
export type BoxOptions = {
  ...
  mode?: ParseMode;
  style?: any; // If a `style` option is provided, a `mode` must also be provided.
...
export class Box {
  ...
  constructor(content: null | number | string | Box | (Box | null)[], options?: BoxOptions) {
    if (typeof content === 'number') this.value = String.fromCodePoint(content);
    else if (typeof content === 'string') this.value = content;
    else if (Array.isArray(content)) this.children = content.filter(x => x !== null) as Box[];
    ...
    if (options?.maxFontSize !== undefined) this.maxFontSize = options.maxFontSize;
  }
  ...
  selected(isSelected: boolean): void {
    ...
    if (this.children) for (const child of this.children) child.selected(isSelected);
  }
  ...
  setStyle(prop: BoxCSSProperties, value: string | number | undefined, unit?: string): void {
    // console.assert(
  ...
  get left(): number {
    if (this.cssProperties?.['margin-left']) return Number.parseFloat(this.cssProperties['margin-left']);
  ...
  wrap(
    ...
    const newSize = context.effectiveFontSize === parent.effectiveFontSize ? undefined : context.effectiveFontSize;
  ...
  tryCoalesceWith(box: Box): boolean {
    ...
    if (!/ML__text/.test(this.classes) && !['mord', 'mbin', 'mrel'].includes(this.type)) return false;
...
function adjustType(root: Box | null): void {
  forEachBox(root, (prevBox: Box, box: Box) => {
    ...
    // >   with Rule 17.
    if (prevBox && prevBox.type === 'mbin' && /mrel|mclose|mpunct|placeholder/.test(box.type)) prevBox.type = 'mord';
  });
}
...
function forEachBoxRecursive(prevBox: Box | null, box: Box, f: (prevBox: Box | null, curBox: Box) => void): Box | null {
  ...
  if (box.children) {
    ...
    for (const child of box.children) childPrev = forEachBoxRecursive(childPrev, child, f);
