...
export function defaultGetDefinition(token: string, parseMode: ParseMode = 'math'): TokenDefinition | null {
  ...
  if (token.startsWith('\\')) {
    ...
    if (!info && token.length === 1) {
      ...
      if (command.startsWith('\\')) return {...defaultGetDefinition(command, 'math')!, command};
      return null;
    }
  } else if (TEXT_SYMBOLS[token]) {
    info = {
