...
function getVListChildrenAndDepth(
  params: VBoxParam
): [children: null | (VBoxChild | VBoxElementAndShift)[] | VBoxChild[], depth: number] {
  ...
  if ('top' in params) {
    ...
    if (typeof firstChild === 'number') throw new Error('First child must be an element.');
 ...
 */
function makeRows(params: VBoxParam): [rows: Box[], height: number, depth: number] {
  ...
  for (const child of children) {
    if (typeof child === 'number') currPos += child;
      ...
      if (child.marginLeft) childWrap.setStyle('margin-left', child.marginLeft, 'em');

      if (child.marginRight) childWrap.setStyle('margin-right', child.marginRight, 'em');
  ...
  if (minPos < 0) {
    ...
    rows = [new Box([vlist, topStrut], {classes: 'vlist-r'}), new Box(depthStrut, {classes: 'vlist-r'})];
...
export class VBox extends Box {
  constructor(content: VBoxParam, options?: {classes?: string; type?: BoxType}) {
...
export function makeLimitsStack(
  ...
  if (options.below && options.above) {
    const bottom =
      metrics.bigOpSpacing5 + options.below.height + options.below.depth + belowShift + base.depth + baseShift;
