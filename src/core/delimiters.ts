...
import {getCharacterMetrics, PT_PER_EM, AXIS_HEIGHT, FONT_SCALE} from './font-metrics';
...
export const LEFT_DELIM = Object.fromEntries(
  Object.entries(RIGHT_DELIM).map(([leftDelim, rightDelim]) => [rightDelim, leftDelim])
);
 ...
 */
function traverseSequence(delim: number, height: number, sequence: DelimiterInfo[], context: Context): DelimiterInfo {
  ...
  for (let i = start; i < sequence.length; i++) {
    ...
    if (sequence[i]!.type === 'small') {
      if (sequence[i]!.mathstyle === 'scriptscriptstyle') heightDepth *= FONT_SCALE[Math.max(1, context.size - 2)]!;
      else if (sequence[i]!.mathstyle === 'scriptstyle') heightDepth *= FONT_SCALE[Math.max(1, context.size - 1)]!;
    }
...
export function makeCustomSizedDelim(
  ...
  if (!delim || delim.length === 0 || delim === '.') return makeNullDelimiter(context, type, type);
...
export function makeLeftRightDelim(
  ...
  depth: number,
  context: Context,
  options?: {classes?: string; style?: any; mode?: ParseMode}
): Box {
