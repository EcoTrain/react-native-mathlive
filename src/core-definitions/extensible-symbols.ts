...
defineFunction(
  ...
  '{:auto}',
  {
    createAtom: (command: string, args: Argument[], style: PrivateStyle, context: GlobalContext): Atom =>
...
defineFunction(
  ['underrightarrow', 'underleftarrow', 'underleftrightarrow', 'underlinesegment', 'undergroup'],
  '{:auto}',
  {
    createAtom: (command: string, args: Argument[], style: PrivateStyle, context: GlobalContext): Atom =>
      new OverunderAtom(command, context, {
  ...
  '[:auto]{:auto}',
  {
    createAtom: (command: string, args: Argument[], style: PrivateStyle, context: GlobalContext): Atom =>
      new OverunderAtom(command, context, {
        ...
        above: (args[1] as Atom[])?.length === 0 ? undefined : (args[1] as Atom[]),
